...
namespace DCTC.AI {
    ...
    public class CablePlacementAgent : IAgent {
        ...
        private float executionCoolDown = 0;
        private float cableCoolDown = 0;
        private int currentCableIndex = -1;
        private Evaluation currentEval;

        private const int expansionDistance = 20;
        private const float executionCoolDownDuration = 120;
        private const float cableCoolDownDuration = 10;
        ...
        private void  Init() {
            ...
            cableCoolDown = RandomUtils.RandomFloat(1, cableCoolDownDuration / 2f, company.Game.Random);
        }
        ...
        public float Score(float deltaTime) {
            ...
            if (executionCoolDown <= 0) {
                ...
                if (eval.costEstimate > company.Money) {
                    executionCoolDown = executionCoolDownDuration / 4f;
                    return 0;
                }
        ...
        private float AreaPotentialScore(TileRectangle area, MapConfiguration map, Dictionary<TilePosition, Customer> customers) {
            float score = 0;
        ...
        private Direction BestDirection(IEnumerable<TilePosition> positions, int howFar, out float bestScore) {
            ...
            Dictionary<Direction, float> scores = new Dictionary<Direction, float>() {
                { Direction.North, AreaPotentialScore(north, map, customers) },
                { Direction.South, AreaPotentialScore(south, map, customers) },
                { Direction.East,  AreaPotentialScore(east, map, customers)  },
                { Direction.West,  AreaPotentialScore(west, map, customers)  },
            };
        ...
        private Evaluation Evaluate() {
            ...
            if (company.Networks.Count == 0) {
                ...
                bestDirection = BestDirection(company.HeadquartersConnectors, expansionDistance, out eval.bestScore);
                ...
                foreach (Network network in company.Networks) {
                    ...
                    Direction direction = BestDirection(positions, expansionDistance, out directionScore);
